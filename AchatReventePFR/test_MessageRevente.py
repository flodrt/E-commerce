...
class AchatReventeTestConfirmationStockBudgetMessage(unittest.TestCase):
    ...
    def test_achat_revente_test_confirmation_stock_budget_message(self):
        ...
        # SCREENSHOT 
        driver.save_screenshot('.\TEST_ProjetFilRouge\AchatReventePFR\Screenshots\MessageConfVente.png') 
        #Recuperation message confirmation de vente
        ...
        #TEST MESSAGE CONFIRMATION VENTE 
        print("----------------------------------------------------------------")
        print (msgConfirmationV)
        ValeurAttendue="Congratulations! You sold IPhone 15 back to market!"
        print (ValeurAttendue)
        print ("------------------------------------------------")

        # ASSERT
        self.assertEqual (msgConfirmationV, ValeurAttendue  )
